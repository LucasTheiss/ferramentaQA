document.addEventListener('DOMContentLoaded', () => {
    let items = initialItems.map(item => ({ id: item.id.toString(), question: item.question }));
    let severityLevels = initialLevels.map(level => ({ id: level.id.toString(), name: level.nome, time: level.tempo }));
    ...
    function addSeverityLevel() {
        ...
        if (name && time > 0) {
            ...
            severityNameInput.value = ''; severityTimeInput.value = '';
    ...
    function removeSeverityLevel(id) {
        severityLevels = severityLevels.filter(level => level.id.toString() !== id.toString());
        renderSeverityLevels();
    }
    ...
    function createItemElement(item, index) {
        ...
        div.innerHTML = `
            ...
            <div class="form-group item-question-group">
                ...
                <textarea class="textarea-field item-input" data-field="question" placeholder="Escreva a pergunta..." rows="2">${item.question}</textarea>
            </div>`;
    ...
    function handleBackButton(e) {
        e.preventDefault(); 
        Swal.fire({
            title: "Sair sem salvar?", text: "As alterações não salvas serão perdidas.", icon: "warning",
    ...
    function submitChecklist() {
        ...
        console.log("Atualizando Checklist:", checklistData);

        fetch(`/editar/${checklistId}`, {
            method: 'POST',
